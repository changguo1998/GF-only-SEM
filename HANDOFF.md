...
> 创建时间: 2026-07-12
> 最后更新: 2026-07-17（GPU MPI exchange + scatter 修复）
...
|| CUDA 单 GPU | 1000 步 / 2.4s | ⚠ 完成但结果错误 — read_partition_all 清除 ibool → legacy element-local 路径（跨单元无法传播）。CG-SEM 路径需要多 rank 分区 |
...
- **CUDA 单 GPU 全局 DOF 路径**：当前 CUDA 单 GPU 使用 element-local 路径。如需与 MPI CPU 路径完全一致，需实现跨分区 ibool 合并（涉及全局节点编号）
