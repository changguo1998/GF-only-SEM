...
public:
    ...
    void write_cells(const std::vector<int32_t>& cells, const std::vector<int32_t>& cell_types) {
        ...
        for (int64_t i = 0; i < n_cells; ++i)
            be_types[i] = to_big_int32(cell_types[i]);
        fwrite(be_types.data(), sizeof(int32_t), be_types.size(), fp_);
        fputc('\n', fp_);
    }

    void begin_cell_data(int64_t n_cells) { fprintf(fp_, "CELL_DATA %ld\n", (long)n_cells); }
    ...
    void write_scalar_field(const std::string& name, const std::vector<float>& data) {
        ...
        for (size_t i = 0; i < data.size(); ++i)
            be[i] = to_big_float(data[i]);
        fwrite(be.data(), sizeof(float), be.size(), fp_);
        fputc('\n', fp_);
    }

    void begin_point_data(int64_t n_points) { fprintf(fp_, "POINT_DATA %ld\n", (long)n_points); }
