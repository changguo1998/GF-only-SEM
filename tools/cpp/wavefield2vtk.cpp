...
static void print_usage(const char *prog) {
    std::fprintf(stderr, "Usage: %s [--verbose] [--model MODEL.H5] [--config CONFIG.H5] [--vtk-dir DIR]\n", prog);
}
...
int main(int argc, char **argv) {
    ...
    if (dataset_exists(fm.id(), "field/element/is_pml")) {
        ...
        for (size_t i = 0; i < pml.size() && i < (size_t)n_cell; ++i) is_pml[i] = (int8_t)pml[i];
    }
    std::cout << "  Global cells: " << n_cell << ", vertices: " << n_vert << "\n";

    // ── Resolve connectivity ────────────────────────────────────
    if (verbose) std::cout << "[wavefield2vtk] Resolving hexahedral connectivity...\n";
    ...
    for (int64_t ci = 0; ci < n_cell; ++ci) {
        ...
        for (int j = 0; j < 8; ++j) connectivity[ci * 8 + j] = conn[j];
    }
    ...
    for (int di = 0; di < 3; ++di) {
        ...
        if (verbose) std::cout << "  wavefields/" << DIRECTIONS[di] << "/: " << files.size() << " rank files\n";
    }
    ...
    for (const auto &rf : record_paths["x"]) {
        ...
        if (dataset_exists(f.id(), "strain")) {
            ...
            if (!sh.empty() && sh[0] > 0) { n_snapshots = (int)sh[0]; break; }
        }
    }
    if (n_snapshots == 0) { std::cerr << "Error: no snapshots\n"; return 1; }
    ...
    if (verbose) std::cout << "  Snapshot stride: " << stride << "\n";
    ...
    for (int di = 1; di < 3; ++di) {
        for (int ri = 0; ri < n_ranks; ++ri) {
            ...
            if (vids != vertex_id_list[ri]) {
                std::cerr << "Error: vertex ID mismatch in " << record_paths[DIRECTIONS[di]][ri].path << "\n";
                return 1;
            }
        }
    }
    ...
    struct OpenRec { hid_t fid; hid_t strain; };
    ...
    for (int di = 0; di < 3; ++di) {
        for (const auto &rf : record_paths[DIRECTIONS[di]]) {
            ...
            if (fid < 0) { std::cerr << "Error opening " << rf.path << "\n"; return 1; }
            hid_t sid = H5Dopen2(fid, "strain", H5P_DEFAULT);
            if (sid < 0) { std::cerr << "Error opening strain in " << rf.path << "\n"; return 1; }
            open_files[di].push_back({fid, sid});
        }
    }

    // Pre-compute per-rank mapping
    struct RankMap { std::vector<int64_t> gids; };
    ...
    for (int di = 0; di < 3; ++di)
        for (int vi = 0; vi < 6; ++vi)
            strain_field_names.push_back(
                std::string("strain_") + VOIGT_LABELS[vi] + "_" + DIRECTIONS[di]);
    ...
    for (int snap_idx = 0; snap_idx < n_snapshots; ++snap_idx) {
        ...
        for (int di = 0; di < 3; ++di) {
            for (int ri = 0; ri < n_ranks; ++ri) {
                ...
                for (int64_t lvi = 0; lvi < nlv; ++lvi) {
                    ...
                    if (gvid < 0 || gvid >= n_vert) continue;
                    for (int c = 0; c < 6; ++c) {
        ...
        for (int64_t ci = 0; ci < n_cell; ++ci) {
            ...
            for (int j = 0; j < 8; ++j) {
                ...
                if (gvid >= 0 && gvid < n_vert && n_corners[gvid] > 0) {
                    for (int di = 0; di < 3; ++di)
                        for (int c = 0; c < 6; ++c)
                            elem_strain[(di * n_cell + ci) * 6 + c] += dir_strain[di][gvid * 6 + c];
                    cnt++;
                }
            }
            elem_nc[ci] = cnt;
        ...
        char out_path[256];
        std::snprintf(out_path, sizeof(out_path), "%s/wavefield_%d.vtk", vtk_dir.c_str(), step_num);
        if (verbose) std::cout << "[wavefield2vtk] Writing " << out_path << "\n";
        ...
        try {
            ...
            for (int64_t ei = 0; ei < n_hex; ++ei) pml_f[ei] = (float)is_pml[ei];
            vtk.write_scalar_field("PML_flag", pml_f);

            std::vector<float> nrec_f(n_hex);
            for (int64_t ei = 0; ei < n_hex; ++ei) nrec_f[ei] = (float)elem_nc[ei];
    ...
    // Cleanup
    for (int di = 0; di < 3; ++di)
        for (auto &of : open_files[di]) { H5Dclose(of.strain); H5Fclose(of.fid); }
