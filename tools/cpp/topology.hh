...
static const int HEX_FACES[6][4] = {{0, 3, 2, 1}, {4, 5, 6, 7}, {0, 1, 5, 4},
...
inline std::vector<int64_t> resolve_cell_vertices(
    const int64_t* cell_surfaces, const std::vector<std::vector<int64_t>>& surface_edges,
...
inline GllTemplate build_gll_template(int ngll) {
    ...
    for (int i = 0; i < ngll - 1; ++i)
        t.edge_lines.push_back({idx(i, 0, 0), idx(i + 1, 0, 0)});
    // Edge 2-3: along i, j=ngll-1, k=0
    for (int i = 0; i < ngll - 1; ++i)
        t.edge_lines.push_back({idx(i, ngll - 1, 0), idx(i + 1, ngll - 1, 0)});
    // Edge 4-5: along i, j=0, k=ngll-1
    for (int i = 0; i < ngll - 1; ++i)
        t.edge_lines.push_back({idx(i, 0, ngll - 1), idx(i + 1, 0, ngll - 1)});
    // Edge 6-7: along i, j=ngll-1, k=ngll-1
    for (int i = 0; i < ngll - 1; ++i)
        t.edge_lines.push_back({idx(i, ngll - 1, ngll - 1), idx(i + 1, ngll - 1, ngll - 1)});
    // Edge 0-2: along j, i=0, k=0
    for (int j = 0; j < ngll - 1; ++j)
        t.edge_lines.push_back({idx(0, j, 0), idx(0, j + 1, 0)});
    // Edge 1-3: along j, i=ngll-1, k=0
    for (int j = 0; j < ngll - 1; ++j)
        t.edge_lines.push_back({idx(ngll - 1, j, 0), idx(ngll - 1, j + 1, 0)});
    // Edge 4-6: along j, i=0, k=ngll-1
    for (int j = 0; j < ngll - 1; ++j)
        t.edge_lines.push_back({idx(0, j, ngll - 1), idx(0, j + 1, ngll - 1)});
    // Edge 5-7: along j, i=ngll-1, k=ngll-1
    for (int j = 0; j < ngll - 1; ++j)
        t.edge_lines.push_back({idx(ngll - 1, j, ngll - 1), idx(ngll - 1, j + 1, ngll - 1)});
    // Edge 0-4: along k, i=0, j=0
    for (int k = 0; k < ngll - 1; ++k)
        t.edge_lines.push_back({idx(0, 0, k), idx(0, 0, k + 1)});
    // Edge 1-5: along k, i=ngll-1, j=0
    for (int k = 0; k < ngll - 1; ++k)
        t.edge_lines.push_back({idx(ngll - 1, 0, k), idx(ngll - 1, 0, k + 1)});
    // Edge 3-7: along k, i=0, j=ngll-1
    for (int k = 0; k < ngll - 1; ++k)
        t.edge_lines.push_back({idx(0, ngll - 1, k), idx(0, ngll - 1, k + 1)});
    // Edge 2-6: along k, i=ngll-1, j=ngll-1
    for (int k = 0; k < ngll - 1; ++k)
        t.edge_lines.push_back({idx(ngll - 1, ngll - 1, k), idx(ngll - 1, ngll - 1, k + 1)});

    // 6 faces × (ngll-1)² QUADs
    // Face 0 (z=0): along i,j
    for (int i = 0; i < ngll - 1; ++i)
        for (int j = 0; j < ngll - 1; ++j)
            t.face_quads.push_back(
                {idx(i, j, 0), idx(i + 1, j, 0), idx(i + 1, j + 1, 0), idx(i, j + 1, 0)});
    // Face 1 (z=ngll-1): along i,j
    for (int i = 0; i < ngll - 1; ++i)
        for (int j = 0; j < ngll - 1; ++j)
            t.face_quads.push_back({idx(i, j, ngll - 1), idx(i + 1, j, ngll - 1),
                                    idx(i + 1, j + 1, ngll - 1), idx(i, j + 1, ngll - 1)});
    ...
    for (int i = 0; i < ngll - 1; ++i)
        for (int k = 0; k < ngll - 1; ++k)
            t.face_quads.push_back({idx(i, ngll - 1, k), idx(i + 1, ngll - 1, k),
                                    idx(i + 1, ngll - 1, k + 1), idx(i, ngll - 1, k + 1)});
    ...
    for (int j = 0; j < ngll - 1; ++j)
        for (int k = 0; k < ngll - 1; ++k)
            t.face_quads.push_back({idx(ngll - 1, j, k), idx(ngll - 1, j + 1, k),
                                    idx(ngll - 1, j + 1, k + 1), idx(ngll - 1, j, k + 1)});

    // (ngll-1)³ sub-volume HEXs
    for (int i = 0; i < ngll - 1; ++i)
        for (int j = 0; j < ngll - 1; ++j)
            for (int k = 0; k < ngll - 1; ++k)
                t.sub_hexes.push_back({idx(i, j, k), idx(i + 1, j, k), idx(i + 1, j + 1, k),
