...
static RankData make_simple_rank(int n_local_cell, int ngll) {
    RankData rd;
    rd.n_local_cell = n_local_cell;
    rd.ngll = ngll;
    for (int i = 0; i < n_local_cell; ++i) {
        rd.local_cell_ids.push_back(i + 1);
    }
    return rd;
}
...
TEST_CASE("Source location at GLL node", "[source]") {
    ...
    std::vector<double> coords(n_local_cell * n_node * 3, 0.0);
    std::vector<double> dxi_dx(n_local_cell * n_node * 9, 0.0);
    ...
    bool found = src.locate(0.5, 0.5, 0.5, coords, dxi_dx, n_local_cell, ngll);
...
TEST_CASE("Source apply preserves force magnitude", "[source]") {
    ...
    auto rd = make_simple_rank(n_local_cell, ngll);
