...
inline void newmark_predict(double dt, double beta,
...
int run_forward(const std::string& direction) {
    // Frozen paths relative to CWD
    std::string config_path    = "config.h5";
    std::string partition_dir  = "partitions";
    std::string output_dir     = "";  // wavefields/{direction}/record_{r}.h5 at CWD root
    int rank, nprocs;
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    MPI_Comm_size(MPI_COMM_WORLD, &nprocs);

    if (rank == 0) {
        std::cout << "gf_solver: " << nprocs << " MPI ranks, direction="
                  << direction << std::endl;
    }
    ...
    try {
        ...
        for (int step = 0; step < cfg.nsteps; ++step) {
            ...
            for (int e = 0; e < n_local; ++e) {
                ...
                compute_element_residual(elem_dxi_dx, elem_jac, elem_vp, elem_vs,
                    ...
                    for (int i = 0; i < ngll; ++i) {
                        for (int j = 0; j < ngll; ++j) {
                            for (int k = 0; k < ngll; ++k) {
                                ...
                                strain[strain_offset + 3] = 0.5*(du_dx[0][1]+du_dx[1][0]); // εxy
                                strain[strain_offset + 4] = 0.5*(du_dx[0][2]+du_dx[2][0]); // εxz
                                strain[strain_offset + 5] = 0.5*(du_dx[1][2]+du_dx[2][1]); // εyz
                            }
                        }
                    }
                }
        ...
        if (rank == 0) {
            std::cout << "gf_solver: simulation complete, " << cfg.nsteps
                      << " steps completed." << std::endl;
        }
