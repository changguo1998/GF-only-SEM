...
///   dxi_dx[9 * n_node]  — per GLL node: [dξ/dx, dη/dx, dζ/dx, dξ/dy, dη/dy, dζ/dy, dξ/dz, dη/dz, dζ/dz]
///   jacobian[n_node]    — det(J) per GLL node
///   vp, vs, density     — material properties per GLL node
///
/// GLL quadrature:
...
///   NGLL            — N+1
///
/// @param[in] dxi_dx   [9 * n_node]  — per GLL node: d(xi_i)/dx_j in row-major order (9 values per node)
...
/// @param[in] u        [3 * n_node]  — displacement at all GLL nodes of element (flat x/y/z interleaved)
