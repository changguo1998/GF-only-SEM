...
- **Project structure**:
  ...
  forward/— C++17: elastic physics library (libgf_elastic) + MPI solver executable
...
Notes: `/field/cell/tau_sigma`, `/field/cell/tau_epsilon_mu`, and `/field/cell/tau_epsilon_kappa` are written when SLS attenuation is active. `tau_sigma` carries the `n_sls` and `f0_Hz` attributes needed to reconstruct the analytical complex modulus. No `direction`; runtime CLI sets it.
...
- **Mass computation**: After material interpolation (ρ needed for lumped mass).
  - **CPML precompute**: Tag PML elements and write C-PML profiles (K/d/α per direction) and convolution coefficients (α/β 9 each, Ā₁…Ā₅, A₆…A₂₃). Both displacement-based correction (Ā₁…Ā₅) and strain-based correction (A₆…A₂₃) implemented. See [`docs/design/cpml.md`](cpml.md).
...
- **PML damping**: Displacement-based C-PML (Wang et al. 2006, Xie et al. 2014) with accel correction (Ā₁…Ā₅, 3 memory vars/node) AND strain-based correction (A₆…A₂₃, 18 memory vars/node). Both fully implemented. Replaces old `v -= d·v`. See [`docs/design/cpml.md`](design/cpml.md).
