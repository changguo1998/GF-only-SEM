...
| 维度 | SPECFEM3D | fix-plan | ✓/✗ |
|------|-----------|--------|-----|
| 读取全局位移 | `displ(:, iglob)` where `iglob = ibool(i,j,k,ispec)` | `gather_from_global(displacement, ibool) → elem_displacement` | ✓ 等价 |
| 单元核 | `-K*displ` 使用 GLL 导数矩阵 `hprime_xx`, `hprimewgll_xx` | `K_e * u_tilde` 使用相同 GLL 导数 `dxi_dx`, `D_mat` | ✓ |
| 写入全局 | 直接 `accel(:,iglob) += contribution` — 无独立 scatter 函数 | `scatter_to_global(elem_residual, ibool) → global_residual` — 显式 scatter | ✓ 等价 |
...
| 质量数组 | `rmass(iglob)` — 预求逆 (× 而非 ÷) | `global_mass[iglob]` — 直接除 | ✓ 等价 |
...
| 注入方式 | `accel(iglob) += sourcearrays * stf` — 通过 ibool 直接写入全局 | 写入 `elem_residual` → `scatter_to_global` 分发 | ✓ 等价 |
| 修改需求 | — | **无需修改 source.cpp** — 注入 elem_residual 然后 scatter | — |
...
| 维度 | SPECFEM3D | fix-plan | ✓/✗ |
|------|-----------|--------|-----|
| 位移读取 | `displ(:, ibool(i,j,k,ispec))` — 通过 ibool 从全局读取 | `gather_from_global(displacement, ibool) → elem_displacement` | ✓ 等价 |
...
compute_forces_viscoelastic_calling         gather_from_global(u_tilde)
  ├─ [phase 1] 计算外力 → accel(iglob) +=       → elem_displacement
  ├─ 加载源 → accel(iglob) +=
  ├─ MPI 异步发送 accel 边界                  compute_element_residual
  ├─ [phase 2] MPI 接收 → accel += 接收缓存      → elem_residual
  ├─ 计算外力 → accel(iglob) +=
  ├─ PML → accel (C-PML)                      PML on global velocity [简化]
  └─ accel(iglob) *= rmass(iglob) [质量除法]   加载源 → elem_residual

update_veloc_elastic                         scatter_to_global(elem_residual)
  └─ v = v + dt/2*a         [校步]            → global_residual

                                              exchange_halo(global_residual)
