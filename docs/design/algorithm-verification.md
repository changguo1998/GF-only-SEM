...
> 检查日期: 2026-07-12
> 审查范围: `docs/_archive/plans/2026-07-12-cg-sem-assembly-fix.md` 全部 5 Phase, 17 Tasks
...
## 5. PML 阻尼 (Task 2.4) — 已升级为 C-PML

| 维度 | SPECFEM3D | 当前实现 | ✓/✗ |
|------|-----------|--------|-----|
| 模型 | C-PML — 分裂场卷积完美匹配层 | C-PML — 非对称应力 + 48 记忆变量/节点 | ✓ 与 SPECFEM3D 一致 |
| 加速修正 | Ā₁…Ā₅ + 位移记忆卷积 | 同左 | ✓ |
| 应力修正 | A₆…A₂₃ + 应变记忆卷积 (三组非对称) | 同左 (compute_pml_non_symmetric_stress) | ✓ |
| 参数分离 | pml_set_local_dampingcoeff.f90:1378-1833 | \_separate_pml_parameters | ✓ |
| lx/ly/lz α 记忆 | pml_compute_memory_variables.f90:269-287 | 12 额外记忆/节点 | ✓ |
| K_MAX_PML | 1.0 (源码默认) / 14 (推荐) | 1.0 | ✓ |

> 原文档标注"有意简化"为速度阻尼。2026-07-22 已完成完整 C-PML 实现 (8+ commits)，
> 与 SPECFEM3D 逐项对标，缩放后波形与 Lamb 解析解相关度 99.1%（残余 ~3× 系统性 SEM 离散化因子 — 参见 [`docs/bugs.md`](../bugs.md)）。
...
| PML | ✓ 完整 C-PML (48 记忆变量/节点, 非对称应力, 参数分离) — 2026-07-22 升级 |
| 双阶段重叠 | — 未实现 (单阶段正确, 无计算/通信重叠优化) |

**结论**: fix-plan 中所有影响**正确性**的算法设计与 SPECFEM3D 参考实现一致。PML 已从速度阻尼升级为完整 C-PML (2026-07-22), 与 SPECFEM3D 逐项对标。无双阶段优化不影响结果正确性。
