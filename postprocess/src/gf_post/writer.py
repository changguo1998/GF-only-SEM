...
STRAIN_NAMES = ["strain_xx", "strain_yy", "strain_zz", "strain_xy", "strain_xz", "strain_yz"]
...
class GFWriter:
    ...
    def write(
        ...
        for start in range(0, n_cell, tile_elems):
            ...
            _write_tile(
                tile_path, gll_coords[start:end], time, dt, greens[:, start:end], tile_idx, start
            )
...
def _write_tile(
    ...
    with h5py.File(path, "w") as f:
        ...
        field.create_dataset("greens_tensor", data=tile_greens.astype(np.float32))
        field.create_dataset("coords", data=tile_coords.astype(np.float32))
