...
Supports both array-compatible callables (faster) and scalar callables.
...
def load_and_interpolate(
    ...
    *,
    ...
    # Evaluate at every GLL node (try array-compatible call first)
    vp = _evaluate_field(flat, vp_func, shape)
    vs = _evaluate_field(flat, vs_func, shape)
    density = _evaluate_field(flat, density_func, shape)
...
def _evaluate_field(
    ...
    try:
        ...
        if arr.ndim == 0:
            # Scalar result — broadcast to full shape
