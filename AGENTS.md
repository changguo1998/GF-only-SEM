...
`postprocess/cpp/writer.hpp`, and orphan debug comments in the postprocessor. The serial
and MPI postprocessors now compile one tile-batched `postprocess/cpp/main.cpp`; MPI only
adds round-robin worker scheduling through `GF_POST_MPI`.
...
- **No receivers**: Postprocess uses shallow element-local GLL records, not receiver locations. No receivers.csv, receiver search, or interpolation. See `docs/design-decisions.md`.
...
- **Fixed filenames**: Examples use fixed output names (`model.h5`, `config.h5`, `partition_{r}.h5`, `record_{r}_{step}.h5`, `tile_index_x{i}_y{j}.h5`, `restart_{r}.h5`). No CLI overrides, except input paths.
